...
User = get_user_model()
user, _ = User.objects.get_or_create(email="test2@koraa.cm", defaults={"full_name": "Test", "role": "merchant"})
user.set_password("testpass")
user.save()
Merchant.objects.get_or_create(user=user, defaults={"business_name": "Test Business"})

c = Client()
resp = c.post("/api/v1/auth/login/", {"email": "test2@koraa.cm", "password": "testpass"}, content_type="application/json")
