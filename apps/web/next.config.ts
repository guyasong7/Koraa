...
const nextConfig: NextConfig = {
  ...
  images: {
    ...
    // Both cost CPU once per size, on a cache miss, in the container.
    ...
    // No remotePatterns: every image the app renders is either in public/ or a
    // merchant upload served from the API's own domain through the optimizer's
    // local path. An empty allowlist is the correct one until that changes.
  },
